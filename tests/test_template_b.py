...
from bydelsfakta.templates import TemplateB
...
def test_standards(template_df_latest):
    with_ratios(template, template_df_latest)
    without_ratios(template, template_df_latest)
    values_structure(template, template_df_latest, value_type=dict)
    count_values(template, template_df_latest, 8)


def test_df_to_template_b_all(template_df):
    data_points = ["d1"]
    output = Output(
        values=data_points,
        df=template_df,
